...
function SelectTrigger({ className, children, size, ...props }: SelectTriggerProps) {
  return (
    <SelectPrimitive.Trigger
      ...
      className={cn(selectTriggerVariants({ size }), className, "dark:bg-background dark:text-foreground")}
      {...props}
    >
      {children}
...
function SelectScrollUpButton({ className, ...props }: React.ComponentProps<typeof SelectPrimitive.ScrollUpButton>) {
  return (
    <SelectPrimitive.ScrollUpButton
      ...
      className={cn(
        "relative z-10 flex cursor-default items-center justify-center py-1.5 bg-gradient-to-b from-popover via-popover to-transparent text-muted-foreground",
        className,
      )}
      {...props}
    >
...
function SelectScrollDownButton({
  ...
  return (
    <SelectPrimitive.ScrollDownButton
      ...
      className={cn(
        "relative z-10 flex cursor-default items-center justify-center py-1.5 bg-gradient-to-t from-popover via-popover to-transparent text-muted-foreground",
        className,
      )}
      {...props}
    >
  ...
  return (
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content
        ...
        className={cn(
          "relative z-[400] max-h-96 min-w-[8rem] overflow-hidden rounded-md border border-border bg-popover shadow-md shadow-black/5 text-secondary-foreground data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2",
          position === "popper" &&
...
function SelectItem({ className, children, icon, ...props }: SelectItemProps) {
  ...
  return (
    <SelectPrimitive.Item
      ...
      className={cn(
        "relative flex w-full cursor-default select-none items-center gap-2 rounded-sm py-1.5 text-sm outline-hidden text-foreground hover:bg-accent focus:bg-accent data-disabled:pointer-events-none data-disabled:opacity-50",
