...
/**
 * Canonical button set — prefer these across the app:
 *   variant: default · secondary · outline · ghost · destructive · success · warning · info · link
 *   size:    sm · md · lg · icon
 * `success`/`warning`/`info` are soft, token-driven semantic buttons (tint background + ink text).
 * Reach for them instead of hand-rolling color classes like `bg-white border-green-300 text-green-700`.
 * Other variants (mono/dashed/dim/foreground/inverse) remain for legacy/edge cases.
 */
const buttonVariants = cva(
  "cursor-pointer group whitespace-nowrap focus-visible:outline-hidden inline-flex items-center justify-center has-data-[arrow=true]:justify-between whitespace-nowrap text-sm font-medium ring-offset-background transition-all duration-150 ease-out disabled:pointer-events-none disabled:opacity-60 [&_svg]:shrink-0",
  {
    variants: {
      variant: {
        ...
        success: "bg-success text-success-foreground hover:bg-success/80 data-[state=open]:bg-success/80",
        warning: "bg-warning text-warning-foreground hover:bg-warning/80 data-[state=open]:bg-warning/80",
        info: "bg-info text-info-foreground hover:bg-info/80 data-[state=open]:bg-info/80",
        secondary: "bg-secondary text-secondary-foreground hover:bg-secondary/90 data-[state=open]:bg-secondary/90",
        outline:
          "bg-background text-foreground border border-input hover:bg-accent data-[state=open]:bg-accent dark:bg-background dark:text-foreground",
        dashed:
          "text-foreground border border-input border-dashed bg-background hover:bg-accent hover:text-foreground data-[state=open]:text-foreground",
        ghost:
    ...
    compoundVariants: [
      ...
      },
      {
        variant: "success",
        appearance: "ghost",
        className: "bg-transparent text-success-foreground hover:bg-success/40 data-[state=open]:bg-success/40",
      },
      {
        variant: "warning",
        appearance: "ghost",
        className: "bg-transparent text-warning-foreground hover:bg-warning/40 data-[state=open]:bg-warning/40",
      },
      {
        variant: "info",
        appearance: "ghost",
        className: "bg-transparent text-info-foreground hover:bg-info/40 data-[state=open]:bg-info/40",
      },
      {
        variant: "ghost",
        mode: "icon",
...
function Button({
  ...
  if (disabledTooltip) {
    return (
      <ComposedTooltip content={disabledTooltip}>
        <Comp
          className={cn(
            buttonVariants({ variant, size, className }),
            "cursor-not-allowed border border-neutral-300 bg-neutral-200 text-neutral-400 shadow-none hover:bg-neutral-200 hover:text-neutral-400",
            {
        ...
        >
          {asChild ? (
            ...
            <>
              {children}
              {loading ? <Loading type="spinner" size="xs" className="size-4" /> : icon}
            </>
          )}
        </Comp>
      </ComposedTooltip>
