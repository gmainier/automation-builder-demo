...
>((props, ref) => {
  ...
  return (
    <svg ref={ref} aria-hidden="true" viewBox={`0 0 ${dimension} ${dimension}`} {...otherProps}>
      {size === "xxs" &&
        circlesXXS.map((circle, index) => <circle fill="currentColor" stroke="none" key={index} {...circle} />)}

      {size === "xs" &&
        circlesXS.map((circle, index) => <circle fill="currentColor" stroke="none" key={index} {...circle} />)}

      {size === "sm" &&
        circlesSM.map((circle, index) => <circle fill="currentColor" stroke="none" key={index} {...circle} />)}

      {size === "md" &&
        circlesMD.map((circle, index) => <circle fill="currentColor" stroke="none" key={index} {...circle} />)}

      {size === "lg" &&
        circlesLG.map((circle, index) => <circle fill="currentColor" stroke="none" key={index} {...circle} />)}

      {size === "xl" &&
        circlesXL.map((circle, index) => <circle fill="currentColor" stroke="none" key={index} {...circle} />)}

      {size === "xxl" &&
        circlesXXL.map((circle, index) => <circle fill="currentColor" stroke="none" key={index} {...circle} />)}
    </svg>
  );
});
