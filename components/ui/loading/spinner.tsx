...
>((props, ref) => {
  ...
  return (
    <svg ref={ref} aria-hidden="true" viewBox={`0 0 ${dimension} ${dimension}`} {...otherProps}>
      <circle cx={dimension / 2} cy={dimension / 2} fill="none" r={dimension / 2 - stroke / 2} strokeWidth={stroke} />
      <path d={getDbySize(size)} className="stroke-current" fill="none" strokeLinecap="round" strokeWidth={stroke} />
    </svg>
  );
});
