...
const checkboxVariants = cva(
  `
    group peer cursor-pointer bg-background shrink-0 rounded-md border border-input ring-offset-background
    transition-all duration-150 hover:scale-110 hover:border-primary/50 hover:shadow-sm active:scale-95
  ...
  return (
    <CheckboxPrimitive.Root data-slot="checkbox" className={cn(checkboxVariants({ size }), className)} {...props}>
      <CheckboxPrimitive.Indicator
        forceMount
        className={cn(
          "flex items-center justify-center text-current opacity-0 scale-75 transition-all duration-150 ease-out group-data-[state=checked]:opacity-100 group-data-[state=checked]:scale-100 group-data-[state=indeterminate]:opacity-100 group-data-[state=indeterminate]:scale-100",
        )}
      >
